...
def main():
    ...
    data = pd.read_csv("../../datasets/WELFake_cleaned.csv")
    ...
    for model_name in ["LSTM", "CNN", "CNN_LSTM"]:
        ...
        # folosim validation_data în loc de validation_split
        history = model.fit(X_train_final, y_train_final,
                            validation_data=(X_val, y_val),
                            epochs=10,
                            batch_size=96,  # asigură-te că e divizibil cu nr de GPU-uri
                            callbacks=[early_stop],
                            verbose=2)
        
        batch_size = 96

# Taie setul de test astfel încât să fie multiplu de batch_size
        test_size = (X_test_pad.shape[0] // batch_size) * batch_size
        X_test_trimmed = X_test_pad[:test_size]
        y_test_trimmed = y_test[:test_size]

        loss, accuracy = model.evaluate(X_test_trimmed, y_test_trimmed, batch_size=batch_size, verbose=0)
