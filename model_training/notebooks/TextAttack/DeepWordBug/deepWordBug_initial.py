...
def worker(rank: int, world_size: int, all_chunks):
    ...
    # Importuri ce inițializează CUDA după setare
    import torch
    ...
    # AttackArgs fără device/cuda_device
    attack_args = AttackArgs(
        num_examples=len(all_chunks[rank]),
        log_to_csv=f"deepwordbug_results_gpu_{rank}.csv",
        disable_stdout=False,
        checkpoint_interval=10,
        checkpoint_dir=f"deepwordbug_chkpts_gpu_{rank}"
    )
...
if __name__ == "__main__":
    ...
    # Împarte samples în chunk-uri pentru fiecare GPU
    ...
    # Pornim procesele cu mp.spawn (metoda spawn)
    mp.spawn(
        fn=worker,
        args=(ngpu, chunks),
        nprocs=ngpu,
    ...
    print("=== DeepWordBug a rulat pe toate GPU-urile! ===")
